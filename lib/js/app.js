...
let convertBirthDate = (io_value) => {
    let d = io_value.replaceAll("-","").substring(0,8);
    ...
    _.forEach(mc_persons, function(p) {
        ...
        if(_.isEmpty(io_person)) {
            ...
            updated.push({
                ...
                'Födelsedatum (YYYY-MM-DD)': convertBirthDate(io_person["Födelsedat./Personnr. (ååååmmdd-xxxx)"]), 
                'Kön': convertSex(io_person["Kön"]), 
                'c/o': io_person["Kontaktadress - c/o adress"], 
                'Adress': io_person["Kontaktadress - Gatuadress"], 
                'Postnummer': convertPostnummer(io_person["Kontaktadress - Postnummer"]), 
...
let save_updated = () => {
    ...
    XLSX.writeFile(workbook, "ChangesAndAdditions_"+getShortTimestamp()+".xlsx", {});
}
...
let save_deleted = () => {
    ...
    XLSX.writeFile(workbook, "ToBeRemoved_"+getShortTimestamp()+".xlsx", {});
}
